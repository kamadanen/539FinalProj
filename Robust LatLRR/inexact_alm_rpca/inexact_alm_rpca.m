function [U_hat Sigma_hat V_hat A_hat E_hat iter] = inexact_alm_rpca(D, lambda, tol, max_iter, rho)
...
A_hat = zeros(m, n);
E_hat = zeros(m, n);
mu = 20 / norm_two; % this one can be tuned
mu_bar = mu * 1e9;
tol2 = 1e-5;
norm_D = norm(D, 'fro');
...
U_hat = U(:, 1:svp);
Sigma_hat = diag(diagS(1:svp) - 1/mu);
V_hat = V(:, 1:svp);
